...
void DrawCopter_2D(const double x, const double y, const double MapSizexy,
    ...
    DrawFullCircle(RealToGlCoord_2D(x, MapSizexy), RealToGlCoord_2D(y,
                                                                    MapSizexy), RealToGlCoord_2D(40, MapSizexy),
                   agentcolor);
    /* Dangerous zone */
...
void DrawSensorRangeNetwork_2D(phase_t *PhaseData,
    ...
    if (DrawCircles == true) {
        DrawCircle(RealToGlCoord_2D(ActualAgentsCoordinates[0] - VizParams->CenterX, VizParams->MapSizeXY),
                   RealToGlCoord_2D(ActualAgentsCoordinates[1] - VizParams->CenterY,
                                    VizParams->MapSizeXY),
                   RealToGlCoord_2D(Unit_params->R_C.Value, VizParams->MapSizeXY), RedColor);
    }
    static double NeighboursCoordinates[3];
    static double DifferenceVector[3];
    static double AbsDistance;
    static int lost = 1; // initialisé à perdu
    ...
    for (i = 0; i < PhaseData[0].NumberOfAgents; i++) {
        if (i != WhichAgent && PhaseData[Now].Laplacian[WhichAgent][i] > Unit_params->sensitivity_thresh.Value) {
            lost = 0; //pas perdu !!!!!!
        }
    }
                ...
                if (PhaseData[Now].Laplacian[WhichAgent][i] > Unit_params->sensitivity_thresh.Value &&
                    ...
                    for (j = 0; j < obstacles.o_count; j++) {
                        ...
                        NumberOfIntersections =
                                IntersectionOfSegmentAndPolygon2D(Intersections,
                                PhaseData[Now].Coordinates[WhichAgent],
                                PhaseData[Now].Coordinates[i],
                        ...
                        if (NumberOfIntersections == 2) {
                            ...
                            DrawCircle(RealToGlCoord_2D(Intersections[0][0] - VizParams->CenterX,
                                                        VizParams->MapSizeXY),
                                       RealToGlCoord_2D(Intersections[0][1] - VizParams->CenterY,
                                                        VizParams->MapSizeXY),
                                       RealToGlCoord_2D(1000, VizParams->MapSizeXY), RedColor);

                            DrawCircle(RealToGlCoord_2D(Intersections[1][0] - VizParams->CenterX,
                                                        VizParams->MapSizeXY),
                                       RealToGlCoord_2D(Intersections[1][1] - VizParams->CenterY,
                                                        VizParams->MapSizeXY),
                                       RealToGlCoord_2D(1000, VizParams->MapSizeXY), RedColor);
                    ...
                    ArrowCenterY = (ActualAgentsCoordinates[1] + NeighboursCoordinates[1]) * 0.5 - VizParams->CenterY;
                    ...
                    DrawThinArrow(RealToGlCoord_2D(ArrowCenterX, VizParams->MapSizeXY),
                                  ...
                                  RealToGlCoord_2D(VectAbs(DifferenceVector) - 60,
                                   VizParams->MapSizeXY), RealToGlCoord_2D(30,
                                   VizParams->MapSizeXY), angle, yellow);
