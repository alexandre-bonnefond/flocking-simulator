    ...
    while (fgets(line, sizeof(line), InputFile) != NULL) {
        ...
        if (*start == ';' || *start == '#') {
            ...
            if (strcmp(ReadedName, "tau_PID_XY") == 0) {
                ...
                NumberOfReadedNames++;
            } else if (strcmp(ReadedName, "Freq") == 0) {
                UnitParams->freq.Value = atof(ReadedValue);
                NumberOfReadedNames++;
...
void SaveUnitModelParamsToFile(FILE * OutputFile_Unit,
    ...
    fprintf(OutputFile_Unit, "# Emitting frequency (GHz)\n");
    fprintf(OutputFile_Unit, "Freq=%lf\n",
            UnitParams->freq.Value);
